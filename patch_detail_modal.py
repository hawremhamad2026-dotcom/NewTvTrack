...
# 1. Update imports
content = content.replace("fetchEpisodeImdbId } from '../tmdb';", "fetchEpisodeImdbId, fetchKurdcinemaSearch, fetchKurdcinemaComments } from '../tmdb';")

# 2. Update state definitions for ReviewTabs
content = content.replace("useState<'written' | 'imdb'>('written')", "useState<'written' | 'imdb' | 'kurdcinema'>('written')")
content = content.replace("useState<'trakt' | 'imdb'>('trakt')", "useState<'trakt' | 'imdb' | 'kurdcinema'>('trakt')")

# Add Kurdcinema specific states
states_injection = """
  // Kurdcinema States
...
"""
content = re.sub(r'(const \[activeEpisodeReviewTab.*?\n)', r'\1' + states_injection, content)

# 3. Add kurdcinema UI in main reviews section
main_kurdcinema_ui = """
                      ...
                      <input 
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            ...
                            fetchKurdcinemaSearch(kurdcinemaSearchQuery, item.type).then(res => {
                        ...
                        {kurdcinemaSearchResults.map((res: any, idx) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              fetchKurdcinemaComments(res.url || res.id, item.type).then(data => {
...
"""
content = content.replace("<span>IMDb Reviews</span>\n              </button>\n            </div>", "<span>IMDb Reviews</span>\n              </button>\n" + main_tab_injection + "            </div>")
content = re.sub(r'(<div className="py-12 flex flex-col items-center justify-center gap-3 bg-zinc-900/10 border border-white/5 rounded-2xl">\s*<div className="w-8 h-8 border-4 border-yellow-500/20 border-t-yellow-500 rounded-full animate-spin" />\s*<span className="text-sm text-zinc-400 font-medium animate-pulse">Fetching IMDb reviews...</span>\s*</div>\s*\)\s*:\s*imdbReviews.length === 0\s*\?\s*\([\s\S]*?\)\s*:\s*\([\s\S]*?\)\s*\}\s*</div>\s*\))', r'\1' + main_kurdcinema_ui, content)

episode_tab_injection = """
                      <button
...
content = content.replace("IMDb\n                      </button>\n                    </div>\n                  </div>", "IMDb\n                      </button>\n" + episode_tab_injection + "                    </div>\n                  </div>")
